...
uses FireDAC.Comp.Client, FireDAC.DApt, FireDAC.Stan.Param, System.JSON, System.StrUtils,
System.SysUtils, DataSet.Serialize, DAO.Connection;

type TUsuarioEndereco = class
  private
    FConn: TFDConnection;
...
begin
  ...
  try
    ...
    begin
      ...
      SQL.Add('values(:id_usuario, :endereco, :complemento, :bairro, :cidade, :uf, :cep, :ind_padrao, :cod_cidade)');
...
procedure TUsuarioEndereco.DefinPadrao;
var
  qry: TFDQuery;
begin
  Validate('DefinPadrao');
...
begin
  if (ID_USUARIO <= 0) and MatchStr(operacao, ['Listar', 'Inserir', 'Editar', 'Excluir', 'DefinPadrao']) then
  raise Exception.Create('ID de Usuário não informada!');
  ...
  if (COD_CIDADE.IsEmpty) and MatchStr(operacao, ['Inserir', 'Editar']) then
    raise Exception.Create('Cód. da cidade não informado!');

  if (ID_ENDERECO <= 0) and MatchStr(operacao, ['Editar', 'Excluir', 'DefinPadrao']) then
    raise Exception.Create('ID do endereço não informado!');
