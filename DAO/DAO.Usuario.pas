...
begin
  ...
  try
    ...
    try
      ...
      begin
        FConn.Rollback;
        raise Exception.Create('Erro na inserção dos dados: ' + E.Message);
      end;
    end;
  finally
    qry.Free;
...
begin
  ...
  if (NOME.IsEmpty) and MatchStr(operacao, ['Inserir', 'Editar']) then
    raise Exception.Create('Nome de Usuário não informado!');

  if (ENDERECO.IsEmpty) and MatchStr(operacao, ['Inserir']) then
    raise Exception.Create('Endereço não informado!');

  if (COMPLEMENTO.IsEmpty) and MatchStr(operacao, ['Inserir']) then
    raise Exception.Create('Complemento não informado!');
